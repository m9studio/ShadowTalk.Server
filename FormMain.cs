...
namespace M9Studio.ShadowTalk.Server
{
    public partial class FormMain : Form
    {
        ...
        {
            int id = server.@base.Count("SELECT MAX(id) AS num FROM users") + 1;
            ...
            server.@base.Send("INSERT INTO users (id, name, email, password, rsa) VALUES (?, ?, ?, ?, ?)", id, nick, login, password, RSA);
            server.Update();
        }
    }
